...
check_command() {
    ...
    if command_exists "$cmd"; then
        echo "OK ($($cmd --version 2>&1 | head -n 1))"
        return 0
    else
        echo "NOT FOUND"
...
# Essential Tools
check_command "kubectl" "Kubernetes CLI" "See https://kubernetes.io/docs/tasks/tools/install-kubectl/" || all_ok=0
check_command "tsh" "Teleport Client" "See Giant Swarm internal docs or Teleport website" || all_ok=0
check_command "go" "Go compiler (for envctl build)" "See https://go.dev/doc/install" || all_ok=0
